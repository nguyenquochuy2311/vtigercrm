<?php
/*+***********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 *************************************************************************************/
$languageStrings = array(
	// Basic Strings
	'Leads' => "Khách hàng tiềm năng",
	'SINGLE_Leads' => "Khách hàng tiềm năng",
	'LBL_RECORDS_LIST' => "Danh sách Khách hàng tiềm năng",
	'LBL_ADD_RECORD' => "Thêm Khách hàng tiềm năng",

	// Blocks
	'LBL_LEAD_INFORMATION' => "Chi tiết KH Tiềm năng",
	'LBL_CUSTOM_INFORMATION' => "Tùy chỉnh thông tin",
	'LBL_ADDRESS_INFORMATION' => "Địa chỉ chi tiết",
	'LBL_DESCRIPTION_INFORMATION' => "Mô tả chi tiết",

	//Field Labels
	'Lead No' => "Số khách hàng tiềm năng",
	'Company' => "Công ty",
	'Designation' => "chỉ định",
	'Website' => "Trang web",
	'Industry' => "Ngành nghề",
	'Lead Status' => "Trạng thái khách hàng tiềm năng",
	'No Of Employees' => "Số lượng nhân viên",
	'Phone' => "Điện thoại chính",
	'Secondary Email' => "Email phụ",
	'Email' => "Email chính",

	//Added for Existing Picklist Entries

	'--None--'=>"--Không có--",
	'Mr.'=>"Ông",
	'Ms.'=>"Bệnh đa xơ cứng.",
	'Mrs.'=>"Bà.",
	'Dr.'=>"Tiến sĩ",
	'Prof.'=>"Giáo sư",

	//Lead Status Picklist values
	'Attempted to Contact'=>"Đã cố gắng liên hệ",
	'Cold'=>"Lạnh lẽo",
	'Contact in Future'=>"Liên hệ trong tương lai",
	'Contacted'=>"Đã liên hệ",
	'Hot'=>"Nóng",
	'Junk Lead'=>"Chì rác",
	'Lost Lead'=>"Chì bị mất",
	'Not Contacted'=>"Chưa liên hệ",
	'Pre Qualified'=>"Đủ điều kiện trước",
	'Qualified'=>"Đạt tiêu chuẩn",
	'Warm'=>"Ấm",

	// Mass Action
	'LBL_CONVERT_LEAD' => "Chuyển thành khách đã giao dịch",

	//Convert Lead
	'LBL_TRANSFER_RELATED_RECORD' => "Chuyển hồ sơ liên quan tới",
	'LBL_CONVERT_LEAD_ERROR' => "Bạn phải bật Tổ chức hoặc Liên hệ để chuyển đổi Khách hàng tiềm năng",
	'LBL_LEADS_FIELD_MAPPING_INCOMPLETE' => "Ánh xạ trường khách hàng tiềm năng chưa hoàn chỉnh(Cài đặt > Trình quản lý mô-đun > Khách hàng tiềm năng > Ánh xạ trường khách hàng tiềm năng)",
	'LBL_LEADS_FIELD_MAPPING' => "Ánh xạ trường khách hàng tiềm năng",

	//Leads Custom Field Mapping
	'LBL_CUSTOM_FIELD_MAPPING'=> "Ánh xạ dữ liệu chuyển đổi khách hàng tiềm năng",
	'LBL_IMAGE_INFORMATION' => "Ảnh hồ sơ",
	'Lead Image' => "Hình ảnh chì",
);
$jsLanguageStrings = array(
	'JS_SELECT_CONTACTS' => "Chọn Khách hàng đã giao dịch để tiếp tục",
	'JS_SELECT_ORGANIZATION' => "Chọn Tổ chức để tiếp tục",
	'JS_SELECT_ORGANIZATION_OR_CONTACT_TO_CONVERT_LEAD' => "Chuyển đổi yêu cầu lựa chọn Liên hệ hoặc Tổ chức"
);